...
class Score:
    def __init__(self, cell_size, text_color = (0, 0, 0), score_x = 40, score_y = 20):
        ...
        self.golden_apple = pygame.image.load("Images/golden_apple.png").convert_alpha()
        self.golden_apple = pygame.transform.scale(self.golden_apple, (cell_size, cell_size))
    ...
    def draw(self, snake):
        ...
        high_surface = self.font.render(str(self.high_score), True, self.text_color)
        score_rect = score_surface.get_rect(center = self.score_location)
        high_rect = score_surface.get_rect(topleft = (score_rect.x + 75, score_rect.y))
        apple_rect = self.apple.get_rect(midright = (score_rect.x, score_rect.centery))
        golden_rect = self.golden_apple.get_rect(midright = (high_rect.x, high_rect.centery))
        fg_rect = pygame.Rect(apple_rect.left, apple_rect.top, apple_rect.width + score_rect.width + 6,apple_rect.height)
        fg_high_rect = pygame.Rect(golden_rect.left, golden_rect.top, golden_rect.width + high_rect.width + 6, golden_rect.height)
